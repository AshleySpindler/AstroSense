...
08/10 - making this to accept numpy array not TFrecord
...
from tensorflow.keras.layers import Lambda, Input, Conv2D, MaxPooling2D, UpSampling2D, Dense, Flatten, Reshape, Conv2DTranspose, Layer, Concatenate
...
def encoder_model(input_data, cond_data, layers, filters, latent_dims, beta):
    """
    ...
    l = []
    if tf.is_tensor(input_data)==True:
        in1 = Input(tensor=input_data, dtype=tf.float32, name='Image_input') #as we use TFrecords, need input layer
    else:
        in1 = Input(shape=input_data.shape[1:], dtype='float32', name='Image_input')
    in1_ = Flatten(name='Flatten_image')(in1)
    if tf.is_tensor(cond_data)==True:
        in2 = Input(tensor=cond_data, dtype=tf.float32, name='Conditions_input')
    else:
        in2 = Input(shape=cond_data.shape[1:], dtype='float32', name='Conditions_input')
    print(in1, in2)

    con = Concatenate(name='Concat_1')([in1_,in2])
    den = Dense(64*64*1, name='Concat_to_Dense')(con)
    res = Reshape((64,64,1), name='Reshape_image')(den)
    l.append(res)
    ...
    encoder = Model([in1,in2], [z_mean,z_sigma,z], name='encoder')
    inputs = [in1,in2]
    outputs = encoder(inputs)

    return encoder, inputs, outputs

def decoder_model(encoder, cond_data, layers, filters, latent_dims):
    """
    ...
    if tf.is_tensor(cond_data)==True:
        cond_input = Input(tensor=cond_data, dtype=tf.float32, name='Conditions_input_decoder')
    else:
        cond_input = Input(shape=cond_data.shape[1:], dtype='float32', name='Conditions_input_decoder')
    con = Concatenate(name='Concat_2')([latent_inputs,cond_input])
    latent_to_reshape = Dense(flat_dims[-1], activation='relu', name='latent_to_conv')(con)
    reshape_to_up  = Reshape(pool_dims[1:], name='reshape_to_conv')(latent_to_reshape)
    ...
    decoder = Model([latent_inputs,cond_input], l[-1], name='decoder')
...
def autoencoder_model(inputs, decoder, latent):
    autoencoder = Model(inputs = inputs, outputs = decoder([latent,inputs[1]]))
    return autoencoder
